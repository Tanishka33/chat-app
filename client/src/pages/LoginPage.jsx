...
const LoginPage = () => {
  ...
  const onSubmitHandler = (event) => {
    ...
    const dataToSend = {
      fullname: fullName,  // ✅ key must match backend schema
      email,
      password,
      bio
    };

    console.log("🔼 Sending data to backend:", dataToSend); // ✅ log the outgoing data
    ...
    login(currState === "Sign up" ? 'signup' : 'login', dataToSend);
  };
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        {currState === "Sign up" && isDataSubmitted && (
          <textarea onChange={(e) => setBio(e.target.value)} value={bio} rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-500' placeholder='Provide a short Bio'></textarea>
        )}
